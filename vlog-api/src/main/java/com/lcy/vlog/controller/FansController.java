...
public class FansController extends BaseInfoProperties {
    ...
    @PostMapping("follow")
    public GracefulJSONResult follow(@RequestParam String myId,
                                  @RequestParam String vlogerId) {

        // 判断两个id不能为空
        if (StringUtils.isBlank(myId) || StringUtils.isBlank(vlogerId)) {
            return GracefulJSONResult.errorCustom(ResponseStatusEnum.SYSTEM_ERROR);
        }

        // 判断当前用户，自己不能关注自己
        if (myId.equalsIgnoreCase(vlogerId)) {
            return GracefulJSONResult.errorCustom(ResponseStatusEnum.SYSTEM_RESPONSE_NO_INFO);
        }
    ...
    @PostMapping("cancel")
    public GracefulJSONResult cancel(@RequestParam String myId,
                                  @RequestParam String vlogerId) {
    ...
    public GracefulJSONResult queryDoIFollowVloger(@RequestParam String myId,
                                                @RequestParam String vlogerId) {
        return GracefulJSONResult.ok(fansService.queryDoIFollowVloger(myId, vlogerId));
    }

    @GetMapping("queryMyFollows")
    public GracefulJSONResult queryMyFollows(@RequestParam String myId,
    ...
    public GracefulJSONResult queryMyFans(@RequestParam String myId,
                                          @RequestParam Integer page,
                                          @RequestParam Integer pageSize) {
        return GracefulJSONResult.ok(
