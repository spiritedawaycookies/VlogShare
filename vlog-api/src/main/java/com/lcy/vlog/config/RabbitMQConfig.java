...
public class RabbitMQConfig {
    ...
    @Bean
    public Binding binding(@Qualifier(EXCHANGE_MSG) Exchange exchange,//必须用qualifier才能绑定目前容器里的这两个, exchange和queue
                           @Qualifier(QUEUE_SYS_MSG) Queue queue) {

        return BindingBuilder
                .bind(queue)
                .to(exchange)
                .with("sys.msg.*")          // 定义routing key路由规则（就好比requestMapping）
                .noargs();
